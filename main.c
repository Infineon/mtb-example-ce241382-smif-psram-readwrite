...
#define CHIP_SELECT MTB_SERIAL_MEMORY_CHIP_SELECT_1
...
uint32_t xip_start_address = 0x80000000;
...
int main(void)
{
    ...
    printf("\x1b[2J\x1b[;H");
    printf("****************** PSRAM Read and Write ****************** \r\n\n");

    /* Initialize the SMIF as a communication */
    result_smif = Cy_SMIF_Init(SMIF1_HW, &SMIF1_config, SMIF_INIT_TIMEOUT, &smifContext);
    ...
    Cy_SMIF_Enable(SMIF1_HW, &smifContext);

    result = mtb_serial_memory_setup(&serial_memory_obj, CHIP_SELECT, SMIF1_HW, SMIF1_hal_config.clock, &smifContext, &smif1BlockConfig);
    if (result != CY_RSLT_SUCCESS)
    {
        CY_ASSERT(0);
    }
    ...
    printf("\r\n2. Reading back for verification\r\n");
    ...
    /* Prepare the TX buffer and clear Rx buffer */
    for (uint32_t index = 0; index < PACKET_SIZE; index++)
    {
        tx_buf[index] = (uint8_t)index + 0x80;
    }
    memset(rx_buf, 0, PACKET_SIZE);

    /* Write memory in XIP mode */
    uint8_t *location_ptr = (uint8_t *)xip_start_address;
    printf("\r\n3. Write data to memory in XIP mode \r\n");
    for (uint32_t index = 0; index < PACKET_SIZE; index++)
    {
        *location_ptr = tx_buf[index];
        location_ptr++;
    }
    print_array("Written Data", tx_buf, PACKET_SIZE);

    /* Read memory in XIP mode for verification */
    location_ptr = (uint8_t *)xip_start_address;
    printf("\r\n4. Reading back data in XIP mode for verification\r\n");
    for (uint32_t index = 0; index < PACKET_SIZE;  index++)
    {
